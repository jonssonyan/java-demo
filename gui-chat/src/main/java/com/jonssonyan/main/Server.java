...
import com.jonssonyan.view.ServerView;
...
public class Server extends ServerView {
    ...
    public Server() {
        ...
        //发送按钮绑定点击事件
        sendButton.addActionListener(e -> sendAll());

        //广播消息框绑定回车键
        serverMessageTextField.addActionListener(e -> sendAll());
    }
            ...
            try {
                ...
                if (clientNum > 0) {
                    ...
                    for (Map.Entry<String, ClientServiceThread> entry : clientServiceThreads.entrySet()) {
                        ...
                        buffer.append(serviceThread.getUser().description()).append("@");
                        //告诉其他用户此用户在线  
